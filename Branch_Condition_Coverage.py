...
class TestCountClumps(unittest.TestCase):
    ...
    def test_no_clumps(self):
        self.assertEqual(CountClump([1, 2, 2, 3, 4]), 1)  # Condition A (False), B (False), C (False), D (True)
    
    def test_no_clumps_alternate(self):
        self.assertEqual(CountClump([1, 2, 3, 4, 5]), 0)  # Condition A (False), B (False), C (False), D (False)
